...
def getValidInputNumberRange(greaterThan, lessThan, label):
    while True:
        ...
        try:
            if validInputNumberRange == "/x" or validInputNumberRange == "/X":
                return "/x"
...
def getValidSpecificNumberValue(value, label):
    while True:
        validSpecificNumberValue = input(f'Enter {label} : ')
        if validSpecificNumberValue == "/x" or validSpecificNumberValue == "/X":
                return "/x"
        try:
            if int(validSpecificNumberValue) == value:
                break
            print("\nInvalid Input\n")
        except:
            pass
