...
def update_recipe(recipeCatalog):
    print("\n\t\t\t\t*~*~*~ Update a Recipe ~*~*~*\n")
    ...
    while currentRecipe != None:
        if currentRecipe.name.lower()== nameOfRecipe.lower():
            isFound = True
            while True:
                ...
                if choiceOfUpdate == "1":
                    ...
                    print(f"\nName of recipe {currentRecipe.name} was changed to {new_name}\n")
                    currentRecipe.name = new_name
                    break

                elif choiceOfUpdate == "2":
                    print("\nUpdate Recipe Cost\n")
                    new_cost = valid.getValidInputNumberValue(0, "cost")
                    print(f"\nCost of recipe {currentRecipe.name} was updated from {currentRecipe.cost} to {new_cost}\n") 
                    currentRecipe.cost = new_cost 
                    break
                    ...
                    while True:
                        ...
                        if updateIngredientChoice == "1":
                            ...
                            for index, ingredient in enumerate(currentRecipe.ingredients):
                                print(f'[{index+1}] {ingredient.capitalize()}')
                            print()

                            update_index = valid.getValidInputNumberRange(1, len(currentRecipe.ingredients), "index of ingredient to update")

                            updated_ingredient = valid.getValidString("updated ingredient")
                            print(f"\nIngredient {currentRecipe.ingredients[update_index-1]} was updated to {updated_ingredient}\n")
                            currentRecipe.ingredients[update_index-1] = updated_ingredient
                            ...
                            print(f"\nIngredients of recipe {currentRecipe.name} was updated\n")
                    ...
                    while True:
                        ...
                        if updateIngredientChoice == "1":
                            ...
                            for index, step in enumerate(currentRecipe.steps):
                                print(f'[{index+1}] {step.capitalize()}')
                            print()
                             
                            update_index = valid.getValidInputNumberRange(1, len(currentRecipe.steps), "index of recipe step to update")

                            updated_step = valid.getValidString("updated recipe step")
                            print(f"\nRecipe step {currentRecipe.ingredients[update_index-1]} was updated to {updated_step}\n")
                            currentRecipe.steps[update_index-1] = updated_step
                            ...
                            print(f"\nSteps of recipe {currentRecipe.name} was updated\n")
